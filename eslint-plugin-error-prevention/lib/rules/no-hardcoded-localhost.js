export default {
  ...
  create(context) {
    ...
    const filename = context.physicalFilename || context.filename || '<input>';
    ...
    return {
      ...
      Literal(node) {
        ...
        // Skip literals that are arguments to fetch calls - handled by CallExpression
        if (node.parent && node.parent.type === 'CallExpression' &&
            node.parent.callee && node.parent.callee.name === 'fetch') {
          return;
        }
